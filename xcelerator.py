...
COLUMN_ALIASES = {
    ...
    'category': [
        'category', 'product_category', 'segment', 'group'
    ],
    'quantity': [
...
CATEGORY_RULES = {
    'Святкові товари': [
        'christmas', 'xmas', 'santa', 'snow', 'reindeer', 'angel',
        'snowflake', 'tree', 'garland', 'bauble', 'holiday', 'noel'
    ],
    'Кухня та сервірування': [
        'mug', 'cup', 'plate', 'bowl', 'jug', 'glass', 'wine', 'teapot',
        'tea', 'coffee', 'bottle', 'tray', 'pan', 'kitchen', 'cutlery',
        'spoon', 'fork', 'knife', 'dish', 'napkin', 'recipe', 'cake',
        'cookie', 'salt', 'pepper', 'coaster'
    ],
    'Декор для дому': [
        'frame', 'mirror', 'clock', 'vase', 'ornament', 'decoration',
        'decor', 'photo', 'picture', 'wall', 'lantern', 'hook', 'drawer',
        'knob', 'trinket', 'home', 'house', 'sign'
    ],
    'Свічки та освітлення': [
        'candle', 'candles', 't-light', 'tealight', 'holder', 'light',
        'lights', 'fairy lamp', 'lamp'
    ],
    'Зберігання та організація': [
        'bag', 'bags', 'storage', 'box', 'boxes', 'basket', 'organiser',
        'organizer', 'case', 'cases', 'tin', 'jar', 'hanger', 'crate'
    ],
    'Паперові товари та листівки': [
        'card', 'cards', 'wrap', 'wrapping', 'paper', 'notebook', 'diary',
        'pencil', 'pen', 'sticker', 'tag', 'postcard'
    ],
    'Дитячі товари': [
        'toy', 'doll', 'baby', 'child', 'children', 'kids', 'game',
        'jigsaw', 'teddy', 'school', 'lunch box'
    ],
    'Текстиль та аксесуари': [
        'apron', 'blanket', 'throw', 'towel', 'scarf', 'sock', 'socks',
        'umbrella', 'gloves', 'hat', 'purse', 'cosy', 'cushion',
        'cover', 'fabric', 'sewing'
    ],
    'Сад та outdoor': [
        'garden', 'bird', 'watering', 'potting', 'seed', 'outdoor',
        'flower', 'plant'
    ],
    'Подарункові товари': [
        'gift', 'present', 'heart', 'love', 'rose', 'wedding', 'party'
    ],
}
...
def _normalize_text(text: object) -> str:
    ...
    value = re.sub(r'[^a-zа-яіїє0-9\s|+-]+', ' ', value)
    value = re.sub(r'\s+', ' ', value)
    return value.strip()


def _classify_single_text(text: str) -> str:
...
def infer_general_category(text: object) -> str:
    ...
    categories = [_classify_single_text(part) for part in parts]
    category_counts = pd.Series(categories).value_counts()

    if len(category_counts) == 0:
        return 'Інші товари'

    return str(category_counts.index[0])
...
def prepare_transactions_dataframe(raw_df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, str], list[str]]:
    ...
    if line_item_like:
        ...
        if 'product_name' in df.columns:
            agg_map['product_name'] = lambda s: ' | '.join(
                sorted({str(x).strip() for x in s.dropna() if str(x).strip()})
            )
    ...
    source_for_category = None
    if 'category' in tx.columns:
        source_for_category = 'category'
    elif 'product_text' in tx.columns:
        source_for_category = 'product_text'
    elif 'product_name' in tx.columns:
        source_for_category = 'product_name'

    if source_for_category is not None:
        tx['category'] = tx[source_for_category].apply(infer_general_category)
...
def merge_customer_product_stats(transactions: pd.DataFrame, products: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    if 'category' not in products.columns:
        category_mapping = infer_column_mapping(products.columns.tolist())
        if 'category' in category_mapping:
            products = products.rename(columns={category_mapping['category']: 'category'})
...
def add_base_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'transaction_date' in work.columns:
        ...
        work = work.sort_values(
            ...
            if 'transaction_id' in work.columns else
    ...
    work['purchase_frequency'] = _safe_ratio(
        work['customer_order_count'],
        work['customer_tenure_days'] + 1
    ).fillna(0.0)

    work['items_per_order'] = _safe_ratio(
        work['basket_items'],
        work['total_transactions']
    ).fillna(0.0)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Підготовка датасету для прогнозування відтоку клієнтів')
    ...
    print('Генерація базових фіч...')
    df = add_base_features(df)

    if 'churn' not in df.columns or df['churn'].nunique() < 2:
        print(f'Формування churn на горизонті {args.horizon} днів...')
    ...
    print(f'Матриця ознак: {X_path} (рядків: {len(X)}, ознак: {len(X.columns)})')
    print(f'Цільова змінна: {y_path}')
    print(f'Ознаки з категоріями: {featured_path}')