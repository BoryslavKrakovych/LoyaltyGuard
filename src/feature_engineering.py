...
def add_base_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'distinct_products' in work.columns:
        ...
        work['distinct_products'] = (
            ...
            .transform(lambda s: s.astype(str).replace({'<NA>': np.nan}).expanding().apply(lambda _: np.nan))
        )
        work['distinct_products'] = (
            work.groupby('customer_id')['product_id']
            .cumcount() + 1
    ...
    if category_spend_cols:
        ...
        work['distinct_categories'] = (
            ...
            .transform(lambda s: s.fillna('unknown').astype(str).expanding().apply(lambda _: np.nan))
        )
        work['distinct_categories'] = (
            work.groupby('customer_id')['category']
            .cumcount() + 1
