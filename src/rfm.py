...
def build_rfm_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'total_sales' in work_df.columns:
        work_df['rfm_monetary'] = pd.to_numeric(work_df['total_sales'], errors='coerce').fillna(0).clip(lower=0)
        monetary_col = 'rfm_monetary'
    else:
        monetary_col = None
    ...
    rfm = work_df.groupby('customer_id').agg(
        ...
        monetary=(monetary_col, 'sum') if monetary_col else ('transaction_date', 'count'),
    ).reset_index()
